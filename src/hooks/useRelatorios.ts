...
export function useRelatorios(type: ReportType, page: number = 0, size: number = 20) {
    ...
    async function fetchReport() {
      ...
      try {
        ...
        switch (type) {
            ...
            case "BALANCE":
                response = await api.get(`/reports/inventory-balance${queryParams}`);
            ...
            case "LOW_STOCK":
                response = await api.get(`/reports/low-stock-products${queryParams}`);
                setData(response.data.content);
                setPageInfo({
                    totalPages: response.data.totalPages,
                    totalElements: response.data.totalElements,
        ...
        setError("Não foi possível carregar os dados do relatório. Verifique se o backend está rodando.");
      } finally {
        setLoading(false);
      }
    }
