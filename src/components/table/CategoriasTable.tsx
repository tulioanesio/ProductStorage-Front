...
export function CategoriasTable({
  ...
  const columns: ColumnDef<any>[] = [
    { accessorKey: "name", header: "Nome", cell: ({ row }) => <span className="capitalize">{row.original.name}</span> },
    { accessorKey: "size", header: "Tamanho", cell: ({ row }) => <span className="capitalize">{row.original.size}</span> },
    { accessorKey: "packaging", header: "Embalagem", cell: ({ row }) => <span className="capitalize">{row.original.packaging}</span> },
    {
      id: "actions",
      enableHiding: false,
      cell: ({ row }) => {
          ...
          <>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => onEdit?.(cat)}>Editar categoria</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setCategoryToDelete(cat)}>
                  <p className="text-red-500">Excluir categoria</p>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <AlertDialog
              ...
              onOpenChange={(o: boolean) => !o && setCategoryToDelete(null)}
            >
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Confirmar exclusão</AlertDialogTitle>
                  <AlertDialogDescription>
                    Tem certeza que deseja excluir <strong>{cat.name}</strong>? Esta ação é irreversível.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
